...
namespace libsnark {
	...
	r1cs_example<FieldT> gen_r1cs_difficulty_example(int thisBlock = 5)
	{
		protoboard<FieldT> pb;

		pb_variable_array<FieldT> hash;
		pb_variable<FieldT> nbits;

		hash.allocate(pb, 8, "hash");
		nbits.allocate(pb, "nbits");

		difficulty_comparison_gadget<FieldT> diffcmp(pb, hash, nbits, "diff_cmp");
		printf("construction\n");
		...
		for (int i = 0; i < 8; ++i) {
			pb.val(hash[i]) = get_unsigned(sample_headers[thisBlock - 1] + 8 + i * 8);
		}

		pb.val(nbits) = get_unsigned(sample_headers[thisBlock] + 18 * 8);
...
int main(int argc, const char * argv[])
{
    ...
    r1cs_example<Fr<default_r1cs_ppzksnark_pp> > example = gen_r1cs_difficulty_example<Fr<default_r1cs_ppzksnark_pp> >();
