...
header_verifier_gadget<FieldT>::header_verifier_gadget(protoboard<FieldT> &pb,
	...
	timecmp.reset(new unixtime_gadget<FieldT>(pb, timestamp_median, local_input[9], annotation_prefix + " timecmp"));
...
template<typename FieldT>
void generate_r1cs_equal_constraint(protoboard<FieldT> &pb, const pb_linear_combination<FieldT> &A, const pb_linear_combination<FieldT> &B) {
...
void header_verifier_gadget<FieldT>::generate_r1cs_constraints() {
	...
	generate_r1cs_equal_constraint<FieldT>(this->pb, msg_output[8], local_input[9]);
	for (int i = 1; i < 11; ++i) {
		generate_r1cs_equal_constraint<FieldT>(this->pb, msg_output[8 + i], timestamp[i - 1]);
	}
}
...
void header_verifier_gadget<FieldT>::generate_r1cs_witness() {
	...
	for (int i = 0; i < 11; ++i) {
		this->pb.val(msg_output[8 + i]) = i ? this->pb.val(timestamp[i - 1]) : this->pb.val(local_input[9]);
	}
}
